...
class TestTank:
    ...
    def test_geometry(self):
        ...
        init = {'geom.r_int' : 3., 'geom.r_ext' : 4., 'geom.thickness' : 0.1, 'geom.height' : 1.,
                'geom.rho_struct' : 0.1, 'geom.rho_fuel' : 0.2, 'weight_p' : 6., 'w_in' : 0., 'fuel.w_out_max' : 1.}
        
        driver.set_scenario(init = init)

        sys.run_drivers()

        np.testing.assert_allclose(sys.props.Mass(), 0.86 * np.pi + 1., atol=10**(-2))
