...
for ti in time:
    iteration_results = {
        ...
        "r2y": r2[i][1],
        "r2z": r2[i][2],
        "r_then_r2_x": r_then_r2[i][0],
        "r_then_r2_y": r_then_r2[i][1],
        "r_then_r2_z": r_then_r2[i][2],
        "vx": v[i][0],
        "vy": v[i][1],
        "vz": v[i][2],
        "ax": a[i][0],
...
def vector_arrow_3d(x0, x1, y0, y1, z0, z1, color):
    ...
    """
    return Arrow3D(
        x0, x1, y0, y1, z0, z1, mutation_scale=10, lw=1, arrowstyle="-|>", color=color
    )
...
class Animator:
    def __init__(self, simulation_results):
        ...
        self.fig.subplots_adjust(
            left=0.05, bottom=None, right=0.95, top=None, wspace=None, hspace=0.28
        )
    ...
    def visualize(self, i):
        ...
        if i < time_parachute:
            # define vectors
            vectors = [
                vector_arrow_3d(0, 0, 0, row.rx, row.ry, row.rz, "g"),
                vector_arrow_3d(row.rx, row.ry, row.rz, row.rtx, row.rty, row.rtz, "r"),
                vector_arrow_3d(
                    row.rx, row.ry, row.rz, row.indyx, row.indyy, row.indyz, "k"
                ),
                vector_arrow_3d(
                    row.rx, row.ry, row.rz, row.indzx, row.indzy, row.indzz, "k"
                ),
                vector_arrow_3d(
                    row.wind_bx,
                    row.wind_by,
            ...
            vectors = [
                vector_arrow_3d(0, 0, 0, row.r2x, row.r2y, row.r2z, "g"),
                #    vector_arrow_3d(0, 0, 0, row.r1x, row.r1y, row.r1z, 'r'),
                vector_arrow_3d(
                    row.r2x,
                    row.r2y,
                    row.r2z,
                    (row.r1x - row.r2x) * ratio,
                    (row.r1y - row.r2y) * ratio * 5,
                    (row.r1z - row.r2z) * ratio * 5,
                    "r",
                ),
                vector_arrow_3d(
                    row.wind_bx,
        ...
        self.trajectory.set_data(
            self.simulation_results["r_then_r2_x"][:i],
            self.simulation_results["r_then_r2_y"][:i],
        )
