...
class Dynamics(System):
    def setup(self):
        ...
        self.add_inward(
            "av_in", np.zeros(3), desc="Rocket Angular Velocity", unit="1/s"
        )
        ...
        self.add_outward(
            "aa", np.zeros(3), desc="Rocket Angular Acceleration", unit="1/s**2"
        )
    ...
    def compute(self):
        ...
        # Acceleration
        self.a[0] = (
            (self.F[0] + self.Fp[0]) / self.m + g[0] + av[2] * v[1] - av[1] * v[2]
        )
        self.a[1] = (
            (self.F[1] + self.Fp[1]) / self.m + g[1] + av[0] * v[2] - av[2] * v[0]
        )
        self.a[2] = (
            (self.F[2] + self.Fp[2]) / self.m + g[2] + av[1] * v[0] - av[0] * v[1]
        )
