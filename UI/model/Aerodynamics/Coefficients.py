...
class Coefficients(System):
    def setup(self):
        self.add_inward(
            "Coefs_ang", np.zeros(3), desc="Computing artefact, don't worry"
        )

        # Speed
        self.add_inward("v_cpa", np.zeros(3), desc="CPA velocity", unit="m/s")
        self.add_inward(
            "av", np.zeros(3), desc="Rocket Angular Velocity (pqr)", unit="1/s"
        )
        ...
        self.add_outward(
            "S_ref", np.pi * (self.d / 2) ** 2, desc="Reference Surface", unit="m**2"
        )
        self.add_inward(
            "A0", 0.0, desc="Cross section at the top of the body", unit="m**2"
        )
        self.add_inward(
            "Al",
            self.S_ref,
            desc="Cross section at the bottom of the body",
            unit="m**2",
        )
        ...
        self.add_inward(
            "GammaC",
            np.arctan2(self.Xt + 0.5 * (self.Ct - self.Cr), self.s),
            desc="Fin mid-chord sweep angle",
            unit="m",
        )
        ...
        self.add_outward("N", 0.0, desc="Normal force")
        self.add_outward("M", 0.0, desc="Pitch moment")
        self.add_outward("Mroll", 0.0, desc="Roll moment")
        self.add_outward("Xcp", 0.0, desc="CPA position from the rocket top", unit="m")
    ...
    def compute(self):
        ...
        alpha = (
            np.arccos(self.v_cpa[0] / v_norm) if v_norm > 0.1 else 0
        )  # angle d'attaque

        M = v_norm / 340.29  # Mach number
        Beta = np.sqrt(
            abs(M**2 - 1)
        )  # In our case, we will almost always have a subsonic flight with M<1
        ...
        Yma = (
            self.s / 3 * (self.Cr + 2 * self.Ct) / (self.Cr + self.Ct)
        )  # Span wise position of the mean aerodynamic chord
        Afin = (self.Cr + self.Ct) / 2 * self.s  # One sided area
        V = (self.l - self.ln) * self.Al + 1 / 3 * self.ln * np.pi * (
            self.d / 2
        ) ** 2  # Volume of the body

        Cna_one_fin = (2 * np.pi * self.s**2 / self.S_ref) / (
            1 + (1 + (Beta * self.s**2 / (Afin * np.cos(self.GammaC))) ** 2) ** 0.5
        )
        # print("Normal coef for a single fin", Cna_one_fin)

        normalisation_coeff = [1, 1, 1, 1, 0.948, 0.913, 0.854, 0.810, 0.750]
        coeff = (
            normalisation_coeff[self.NFins - 1] if self.NFins <= 8 else 0.750
        )  # coeff to keep the formula valid for any number of fins
        Cna_all_fins = Cna_one_fin * self.NFins / 2 * coeff
        Cna_all_fins = Cna_all_fins * (
            1 + r / (self.s + r)
        )  # We need to take into account the fin-body interference
        # print("interference factor", (1 + r/(self.s + r)))
        # print("Normal coef for 4 fins", Cna_all_fins)
        Cna_body = (
            2 / self.S_ref * (self.Al - self.A0)
        )  # *np.sin(self.alpha)/self.alpha
        # print("Cna body", Cna_body)

        Cma_body = (
            2 / (self.S_ref * self.d) * (self.l * self.Al - V)
        )  # *np.sin(self.alpha)/self.alpha
        ...
        Cm = (
            Cma * alpha
        )  # Check the formula, it is calculated at the top nose here and should be calculated at the CG
        ...
        Xfins = (
            l_t
            + self.Xt / 3 * (self.Cr + 2 * self.Ct) / (self.Cr + self.Ct)
            + 1
            / 6
            * (self.Cr**2 + self.Ct**2 + self.Cr * self.Ct)
            / (self.Cr + self.Ct)
        )
        # print(Xfins - l_t)
        ...
        self.M = (
            0.5 * Cm * self.rho * v_norm**2 * self.S_ref * self.d
        )  # TODO : Finish the calculation by taking into account the moment induced by the fins. It is not mandatory right now, because the moment is calculated by a lever arm technique.

        # Parameters for Roll Moment.
        # Documented at: https://github.com/RocketPy-Team/RocketPy/blob/master/docs/technical/aerodynamics/roll_equations.rst
        λ = self.Ct / self.Cr
        tau = (self.s + r) / r
        ...
        # print(f'{self.av[0]=}')
        Mld = (
            1 / 2 * self.rho * v_norm * self.S_ref * self.d**2 * Cldw * self.av[0] / 2
        )
        self.Mroll = Mlf - Mld
        # print(f'{self.Mroll=}')
        ...
        t = 0.0005  # ailerons de 5mm d'épaisseur
        f_B = self.l / self.d
        c_bar = (
            2 / 3 * (self.Cr + self.Ct - self.Cr * self.Ct / (self.Cr + self.Ct))
        ...
        if R < 10**4:
            ...
            C_f = 0.032 * (R_s / self.l) ** 0.2

        C_f_c = C_f * (1 - 0.1 * M**2)

        C_D_friction = (
            C_f_c
            * ((1 + 1 / (2 * f_B)) * A_wet_body + (1 + 2 * t / c_bar) * A_wet_fins)
            / A_ref
        )
        ...
        C_D_nose_pressure = 0.8 * np.sin(phi) ** 2

        C_D_base_pressure = 0.12 + 0.13 * M**2
        ...
        C_D_0 = (
            1 / A_ref * (A_nose * C_D_nose_pressure + A_fin * C_D_fins) + C_D_friction
        )
