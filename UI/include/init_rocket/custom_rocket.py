...
class CustomRocket:
    ...
    def __init__(
        ...
        booster,
        additional_mass=[],
        textures=[None for _ in range(5)],
        dictionnary={},
    ) -> None:
        self.nose = nose
        self.body = body
    ...
    def fromDict(cls, dictio):
        ...
        """
        nose = mc.nose(
            shape_function=dictio["nose_type"],
        ...
        body = mc.empty_cylinder(
            outer_radius=dictio["tube_radius"],
        ...
        volume = (
            np.pi * (motor["diameter"] * 1e-3 / 2) ** 2 * (motor["length"] * 1e-3)
        )  # conversion of mm^3 in m^3
        density = mass / volume
        booster = mc.cylinder(
            radius=motor["diameter"] * 1e-3 / 2,
        ...
        textures = [
            dictio["nose_material"],
            dictio["tube_material"],
            dictio["fins_material"],
            "motor",
        ]

        if dictio["motor_ring"]:
            motor_ring = mc.empty_cylinder(
                outer_radius=dictio["tube_radius"] - dictio["tube_thickness"],
        ...
        for i in range(len(mass_list)):
            ...
            if current_mass["type"] == "cylinder":
                mass = mc.cylinder(
                    radius=current_mass["outer_radius"],
                    height=current_mass["length"],
                    pos=current_mass["position"],
                    density=current_mass["density"],
                )
            else:
                mass = mc.empty_cylinder(
                    outer_radius=current_mass["outer_radius"],
    ...
    def set_textures(self, textures):
        ...
        Textures setter.
        """
        assert len(textures) == len(
            self.component_number
        ), "Textures list must be the same length as the number of component of your rocket. Enter None for unknown textures."
        self.textures = textures

    def show(
        self, method="mpl", opened=True, only_ext=True, cpa=np.zeros(3), cog=np.zeros(3)
    ):
        """
    ...
    def plot(self, only_ext=False, opened=True):
        ...
        if "VSCODE_CWD" in os.environ:
            ...
            raise Exception(
                "Wrong environment : run the code in either vscode or jupyter notebook."
            )
    ...
    def get_cog(self):
        ...
        """
        rocket = self.asList()
        cog, mass = np.zeros(3), 0.0
        for component in rocket:
            _, vmass, vcog, __ = component.stl_mesh.get_mass_properties_with_density(
                component.density
            )
            cog += vmass * vcog
            mass += vmass
        cog /= mass
        cog[cog < 1e-6] = 0.0  # micrometric precision on the cog
        return cog, mass
    ...
    def get_cpa(self):
        ...
        Cna_one_fin = (2 * np.pi * s**2 / Al) / (
            1 + (1 + (s**2 / (Afin * np.cos(GammaC))) ** 2) ** 0.5
        )
        Xfins = (
            l_t
            + Xt / 3 * (Cr + 2 * Ct) / (Cr + Ct)
    ...
    def print_properties(self):
        ...
        mass = (
            input_mass if mass == 0 else mass
        )  # Change the value of the mass to the calculated one if the input is 0
        ...
        print("\n")
        print(
            "Inertia matrix expressed at the COG (kg*m^2) = {0}".format(inertia[0, :])
        )
        print(
            "                                               {0}".format(inertia[1, :])
        )
