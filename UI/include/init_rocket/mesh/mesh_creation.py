...
def half_cylinder(outer_radius=0.5, inner_radius=0.49, height=2, pnum=25, pos=0.0):
    """
    ...
    vertices = np.concatenate(
        (
            np.array([x_bottom, y_out, z_out]).T,
    ...
    rot = R.from_euler("xyz", [np.pi / 2, 0.0, 0.0], degrees=False)
...
def empty_cylinder(
    outer_radius=0.5, inner_radius=0.49, height=2, pnum=50, pos=0.0, density=1e3
):
    """
    ...
    plot_brute_mesh, plot_stl_mesh = half_cylinder(
...
def cylinder(radius=0.5, height=2, pnum=50, pos=0.0, density=1e3):
    """
    ...
    vertices = np.concatenate(
        (
            np.array([x_bottom, y, z]).T,
            np.array([x_top, y, z]).T,
            np.array([[pos, 0, 0]]),
            np.array([[height + pos, 0, 0]]),
        )
    )
    ...
    return Solid(
        brute_mesh=brute_mesh,
        stl_mesh=cylinder_mesh,
        plot_brute_mesh=brute_mesh,
        plot_stl_mesh=cylinder_mesh,
        density=density,
    )


def fins(
    Cr=0.2, Ct=0.1, Xt=0.1, s=0.15, thick=0.01, fnum=4, radius=0.5, pos=0.0, density=1e3
):
    """
    ...
    plot_brute_mesh = Solid.BruteMesh(
        vertices=plot_total_vertices, faces=plot_total_faces
    )
    ...
    plot_fins.rotate([0.5, 0.0, 0.0], np.pi / 2)

    return Solid(
        brute_mesh=brute_mesh,
        stl_mesh=fins,
        plot_brute_mesh=plot_brute_mesh,
        plot_stl_mesh=plot_fins,
        density=density,
    )
...
def half_nose(
    nose_obj, Cpnum=25
):  # Never used in the code because of face orientation problems
    """
    ...
    for j in range(1, Cpnum):
        ...
        out_vertices = np.concatenate(
            (out_vertices, rotation.apply(out_vertices_to_rotate))
        )
        in_vertices = np.concatenate(
            (in_vertices, rotation.apply(in_vertices_to_rotate))
        )
...
def nose(
    ...
    assert (
        shape_function in valid_functions
    ), f"Invalid shape function. Valid options are: {', '.join(valid_functions)}"
    ...
    nose_points = eval(
        "nc." + shape_function + "(x, radius, length, C=curve_param, thick=thick)"
    )
